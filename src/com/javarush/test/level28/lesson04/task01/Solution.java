...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory {
        ...
        private AtomicInteger groupNumber = new AtomicInteger();
        private AtomicInteger threadNumber = new AtomicInteger();

        {
            countInstance.incrementAndGet();
            groupNumber.set(countInstance.get());
        }

        @Override
        public Thread newThread(Runnable r) {
            threadNumber.incrementAndGet();
            String nameThreadGroup = Thread.currentThread().getThreadGroup().getName();
            String result = String.format("%s-pool-%d-thread-%d", nameThreadGroup, groupNumber.get(), threadNumber.get());
            return new Thread(r, result);
        }
    }
}
